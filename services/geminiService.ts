...
export class GeminiService {
  ...
  async analyze(
    ...
    // Default model is Pro for complex reasoning and chat
    ...
    // If search is enabled, we MUST use Flash per requirements
    if (useSearch) {
      model = 'gemini-3-flash-preview';
      config.tools = [{ googleSearch: {} }];
      // When using tools like search, we often don't want strict JSON if it's a general query
      // but Beagle depends on it for visualization. We'll attempt to keep it.
    ...
    const systemInstruction = `
      You are Beagle, an expert AI Data Scientist and Research Assistant.
