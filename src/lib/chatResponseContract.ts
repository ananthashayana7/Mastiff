...
export function buildContractFallbackSummary(inputPrompt: string, hasCharts: boolean, hasAnalysisCode: boolean): string {
  ...
  const chartStatus = hasCharts
    ? 'Interactive visuals are available below, but they should support the written call rather than drive it.'
    : 'No supporting visuals were generated in this pass, so the answer should stay focused on evidence and current drivers.';
  ...
  const reliabilityStatus = hasAnalysisCode
    ...
    : 'This pass stayed light on computation, so treat the conclusion as directional until a narrower rerun produces deterministic evidence.';

  if (requiresExactlyThreeBullets) {
    return [
      `1) ${reliabilityStatus}`,
      `2) ${chartStatus}`,
      '3) Action: Focus the rerun on the single largest variance, weakest period, or highest-risk KPI so the next answer produces sharper insight instead of a broader recap.',
    ].join('\n');
  }

  return [
    '**Executive Signal** The response needed a deterministic fallback, so the signal is still useful for triage but not yet sharp enough for a confident operating decision.',
    `1) ${reliabilityStatus}`,
    `2) ${chartStatus}`,
    '3) The biggest risk in a fallback answer is false comfort: direction may be right while the true driver bridge, outlier rows, or KPI concentration remains underexplained.',
    ...
    'Forecast: Forecasting is intentionally de-emphasized in this pass; focus on the current drivers unless a dedicated projection is explicitly requested.',
