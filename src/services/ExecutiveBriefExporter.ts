...
function drawPageHeader(pdf: jsPDF, title: string, subtitle?: string) {
    ...
    pdf.rect(0, 22, PAGE.width, 3, 'F');

    pdf.setFillColor(248, 250, 252);
    pdf.roundedRect(PAGE.marginX, 30, PAGE.width - (PAGE.marginX * 2), 20, 5, 5, 'F');

    pdf.setFont('helvetica', 'bold');
    pdf.setTextColor(255, 255, 255);
    pdf.setFontSize(12.5);
    pdf.text('SPARTA Decision Brief', PAGE.marginX, 13.2);

    pdf.setFont('helvetica', 'bold');
    pdf.setTextColor(17, 24, 39);
    pdf.setFontSize(18.5);
    pdf.text(title, PAGE.marginX + 4, 38.5);

    if (subtitle) {
        pdf.setFont('helvetica', 'normal');
        pdf.setTextColor(71, 85, 105);
        pdf.setFontSize(9);
        pdf.text(subtitle, PAGE.marginX + 4, 45);
    }
}
...
function drawCalloutCard(
    ...
    options?: { titleFontSize?: number; bodyFontSize?: number; lineHeight?: number; }
) {
    ...
    const titleLines = pdf.splitTextToSize(title, PAGE.width - (PAGE.marginX * 2) - 10);
    const bodyLines = pdf.splitTextToSize(text, PAGE.width - (PAGE.marginX * 2) - 10);
    const requiredHeight = Math.max(18, 10 + (titleLines.length * 4.5) + (bodyLines.length * lineHeight));
    const nextY = ensureSpace(pdf, cursorY, requiredHeight + 3);
    ...
    pdf.roundedRect(PAGE.marginX, nextY, PAGE.width - (PAGE.marginX * 2), requiredHeight, 5, 5, 'FD');

    pdf.setFont('helvetica', 'bold');
    pdf.setFontSize(titleFontSize);
    pdf.setTextColor(palette.title[0], palette.title[1], palette.title[2]);
    pdf.text(titleLines, PAGE.marginX + 5, nextY + 4.5, { baseline: 'top' });

    pdf.setFont('helvetica', 'normal');
    pdf.setFontSize(bodyFontSize);
    pdf.setTextColor(palette.text[0], palette.text[1], palette.text[2]);
    pdf.text(bodyLines, PAGE.marginX + 5, nextY + 10.5, { baseline: 'top' });

    return nextY + requiredHeight + 4;
}
...
function addImageBlock(pdf: jsPDF, cursorY: number, imageData: string): number {
    ...
    const maxImageWidth = PAGE.width - (PAGE.marginX * 2);
    const maxImageHeight = PAGE.height - (PAGE.marginY * 2) - 12;
    ...
    pdf.roundedRect(PAGE.marginX, nextY, maxImageWidth, imageHeight + 4, 4, 4, 'F');
    pdf.addImage(imageData, 'PNG', PAGE.marginX + ((maxImageWidth - imageWidth) / 2), nextY + 2, imageWidth, imageHeight);
    return nextY + imageHeight + 4;
}

export const exportExecutiveBriefToPDF = async ({ sessionTitle, messages, activeFiles = [] }: ExportContext) => {
    ...
    try {
        ...
        cursorY = drawCalloutCard(
            ...
            { titleFontSize: 9, bodyFontSize: 14, lineHeight: 6.6 }
        );
        cursorY += 2;
        ...
        if (dataQuality) {
            ...
            cursorY = drawCalloutCard(
                ...
                dataQuality.replace(/^Data Quality:\s*/i, ''),
                {
                    fill: [255, 251, 235],
                    border: [253, 224, 71],
                    title: [146, 64, 14],
                ...
                { bodyFontSize: 10.2, lineHeight: 4.9 }
            );
            cursorY += 2;
        }
        ...
        if (chartImage) {
            ...
            cursorY = drawParagraph(pdf, cursorY, 'Insights and actions lead this brief; charts are placed here so management can validate the signal after reading the narrative.', {
                fontSize: 10.2,
                color: [71, 85, 105],
                lineHeight: 4.8,
            });
            cursorY = addImageBlock(pdf, cursorY + 1, chartImage);
        }
        ...
        pdf.save(`SPARTA_Decision_Brief_${sanitizeFilename(title)}.pdf`);
        return true;
    } catch (error) {
